      ...
      case 'multi-select':
        return (
          <div className="multi-select">
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      setSelectedMultiOptions(selectedMultiOptions.filter(s => s !== option));
                    }
                  }}
                />
                <span>{option}</span>
            ...
            <button
              onClick={() => handleResponse(selectedMultiOptions)}
              className="continue-button"
            >
              {currentQuestion.optional && selectedMultiOptions.length === 0 ? 'Skip' : 'Continue'}
            </button>
          </div>
        );
