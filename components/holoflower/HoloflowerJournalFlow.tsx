...
export function HoloflowerJournalFlow({ isOpen, onClose, checkInData }: HoloflowerJournalFlowProps) {
  ...
  // Default demo data if no checkInData provided
  const defaultCheckInData = {
    facets: [
      { code: 'FAM', name: 'Family', element: 'earth', value: 7.5, angle: 0, color: '#8B7355' },
      { code: 'CAR', name: 'Career', element: 'fire', value: 6.2, angle: 30, color: '#FF6B6B' },
      { code: 'REL', name: 'Relationships', element: 'water', value: 8.3, angle: 60, color: '#4A90E2' },
      { code: 'HLT', name: 'Health', element: 'earth', value: 5.8, angle: 90, color: '#8B7355' },
      { code: 'CRT', name: 'Creative', element: 'air', value: 9.1, angle: 120, color: '#87CEEB' },
      { code: 'SPI', name: 'Spiritual', element: 'aether', value: 7.9, angle: 150, color: '#9B59B6' },
      { code: 'FIN', name: 'Financial', element: 'earth', value: 4.5, angle: 180, color: '#8B7355' },
      { code: 'EMO', name: 'Emotional', element: 'water', value: 7.2, angle: 210, color: '#4A90E2' },
      { code: 'MEN', name: 'Mental', element: 'air', value: 8.0, angle: 240, color: '#87CEEB' },
      { code: 'PUR', name: 'Purpose', element: 'fire', value: 6.8, angle: 270, color: '#FF6B6B' },
      { code: 'COM', name: 'Community', element: 'water', value: 5.5, angle: 300, color: '#4A90E2' },
      { code: 'GRW', name: 'Growth', element: 'aether', value: 8.7, angle: 330, color: '#9B59B6' }
    ],
    coherence: 0.72,
    timestamp: new Date().toISOString(),
    signature: 'Creative-Spiritual-Growth'
  };

  const flowData = checkInData || defaultCheckInData;

  // Pre-populate journal with holoflower insights
  useEffect(() => {
    if (isOpen) {
      const highElements = flowData.facets.filter((f: any) => f.value > 7);
      const lowElements = flowData.facets.filter((f: any) => f.value < 3);
      const balancedElements = flowData.facets.filter((f: any) => f.value >= 5 && f.value <= 7);

      // Generate visual snapshot data URL
      const snapshotDataURL = generateSnapshotDataURL(flowData.facets, flowData.coherence);
      ...
      prompt += `Overall Coherence: ${Math.round(flowData.coherence * 100)}%\n`;
      prompt += `Field Signature: ${flowData.signature}\n\n`;
      prompt += `💭 Reflections:\n`;
                  ...
                  >
                    ...
                    <div className="mb-6 p-4 bg-white/5 rounded-xl border border-white/10">
                      ...
                      <div className="flex justify-center">
                        <HoloflowerSnapshot
                          facets={flowData.facets}
                          coherence={flowData.coherence}
                    ...
                    <div className="space-y-4">
                      ...
                      <textarea
                        ...
                        placeholder="Reflect on your holoflower reading... (Press Enter to save, Shift+Enter for new line)"
                      />
