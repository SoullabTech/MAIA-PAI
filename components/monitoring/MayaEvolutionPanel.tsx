...
interface MayaEvolutionPanelProps {
  evolutionData?: any;
  isLoading?: boolean;
}

export default function MayaEvolutionPanel({ evolutionData, isLoading }: MayaEvolutionPanelProps) {
  const [mayaData, setMayaData] = useState<MayaEvolutionData | null>(null);
  const [isLive, setIsLive] = useState(false);

  useEffect(() => {
    if (evolutionData && !evolutionData.isEmpty) {
      // Use the passed real data
      setMayaData({
        identity: 'Maya-ARIA-1 v1.0.0-sacred',
        stage: getEvolutionStage(evolutionData.consciousnessLevel),
        consciousness: evolutionData.consciousnessLevel || 70,
        exchanges: evolutionData.totalExchanges || 0,
        sacredMoments: evolutionData.sacredMomentsDetected || 0,
        hoursActive: evolutionData.activeHours || 0,
        responseAdaptability: evolutionData.responseAdaptability || 82.5,
        lastActive: new Date(),
        recentEvolutions: evolutionData.recentEvolutions?.map((e: any) => ({
          timestamp: new Date(),
          type: e.type,
          impact: parseFloat(e.impact) || 0,
          details: e.description
        })) || []
      });
      setIsLive(!evolutionData.isEmpty);
    } else {
  ...
  const getEvolutionStage = (consciousness: number): 'Apprentice' | 'Emerging' | 'Developing' | 'Mature' | 'Transcendent' => {
    if (consciousness < 20) return 'Apprentice';
    if (consciousness < 40) return 'Emerging';
    if (consciousness < 60) return 'Developing';
    if (consciousness < 80) return 'Mature';
    return 'Transcendent';
  };
