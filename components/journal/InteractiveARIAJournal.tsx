...
import {
  ...
  ChevronRight, Activity, TrendingUp, Compass, Save, CheckCircle, Plus
} from 'lucide-react';
...
export const InteractiveARIAJournal: React.FC = () => {
  ...
  // Speak response using Web Speech API with Maya's actual voice
  const speakResponse = async (text: string) => {
    // Use Maya's voice system if available
    if (voiceEnabled) {
      try {
        // Try to use Maya's actual voice synthesis
        ...
        console.error('Maya voice synthesis failed, using fallback:', error);
      }
    }
    ...
    try {
      const response = await fetch('/api/journal', {
        ...
        body: JSON.stringify({
          ...
          element: selectedElement,
          emotionalState,
          mayaReflection: mayaResponse?.reflection,
          sacredInsight: mayaResponse?.sacredInsight,
          resonance: mayaResponse?.resonance,
    ...
    <div className="w-full max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-black/30 backdrop-blur-sm border border-white/10 rounded-xl p-4">
        ...
        <textarea
          ...
          placeholder="Begin your sacred reflection... (Cmd/Ctrl+Enter to save)"
      ...
      <div className="flex gap-3 mt-4">
        <motion.button
          ...
          whileHover={{ scale: content.trim() && !isSaving ? 1.02 : 1 }}
          whileTap={{ scale: content.trim() && !isSaving ? 0.98 : 1 }}
        >
        ...
        <motion.button
          onClick={() => {
            ...
            // Keep content but add a separator for continuing throughout the day
            setContent(content + (content.trim() ? '\n\n---\n\n' : ''));
            setMayaResponse(null);
            textareaRef.current?.focus();
          }}
          className="flex-1 py-2.5 rounded-lg font-medium bg-amber-600/20 text-amber-400 border border-amber-400/30 hover:bg-amber-600/30 transition-all flex items-center justify-center gap-2"
      ...
      <div className="mt-2 text-center">
        <p className="text-xs text-white/40">
          💡 Tip: Press <kbd className="px-1.5 py-0.5 bg-white/10 rounded text-white/60">Cmd/Ctrl + Enter</kbd> to save anytime
        </p>
      </div>
