...
async function addJamieCordero() {
  console.log('Adding Jamie Cordero to beta testers...');

  const email = 'jcordero@sacredhearthamden.org';
  const explorerName = 'Jamie';
  const invitationCode = 'SOULLAB-JAMIE';
  ...
  try {
    ...
    if (existingExplorer) {
      ...
      console.log('Jamie Cordero added to beta testers!');
      console.log('Explorer ID:', data[0].explorer_id);
      console.log('Explorer Name:', data[0].explorer_name);
      console.log('Invitation Code:', invitationCode);
      console.log('Jamie can now sign up with passcode: SOULLAB-JAMIE');
    }
