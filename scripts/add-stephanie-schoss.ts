...
async function addStephanieSchoss() {
  console.log('Adding Stephanie Schoss to beta testers...');

  const email = 'stephanie@schoss.com';
  const explorerName = 'Stephanie';
  const invitationCode = 'SOULLAB-STEPHANIE';
  ...
  try {
    ...
    if (existingExplorer) {
      ...
      console.log('Stephanie Schoss added to beta testers!');
      console.log('Explorer ID:', data[0].explorer_id);
      console.log('Explorer Name:', data[0].explorer_name);
      console.log('Invitation Code:', invitationCode);
      console.log('Stephanie can now sign up with passcode: SOULLAB-STEPHANIE');
    }
