...
// Consciousness processing logging
export const consciousnessLogger = {
  start: (userId: string, message: string, context?: Record<string, any>) =>
    logger.startTimer('consciousness.processing').end({ userId, message, ...context }),

  fieldDepth: (userId: string, depth: number) =>
    logger.info('consciousness.field', 'depth_update', { userId, depth }),

  response: (userId: string, responseTime: number, tokens: { input: number, output: number }) =>
...
// Voice synthesis logging
export const voiceLogger = {
  synthesis: (textLength: number, element: string, model: string, duration: number) =>
  ...
  streaming: (mode: 'enabled' | 'disabled', improvement?: number) =>
    logger.info('voice.streaming', mode, improvement ? {
...
export const apiLogger = {
  request: (endpoint: string, userId?: string, method: string = 'POST') =>
    logger.startTimer('api.request'),

  quota: (userId: string, allowed: boolean, reason?: string) =>
    logger.info('api.quota', allowed ? 'allowed' : 'exceeded', { userId, reason }),
